                                ...
                                <tbody>
                                    @foreach($data as $val)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            <td>{{ $val->name}}</td>
                                            <td>{{ \Carbon\Carbon::parse($val->holiday_date)->format('j F, Y')}}</td>
                                            <td>{{ \Carbon\Carbon::parse($val->holiday_date)->format('l')}}</td>
                                            @if (Auth::user()->hasRole('admin'))

                                            <td class="text-end">
																
																<a href="{{ URL::to('holiday/' . $val->id . '/edit') }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                                ...
                                                                <form action="{{ route('holiday.destroy', $val->id) }}" method="POST" id="delete-{{$val->id}}">
  @csrf
  @method("DELETE")
