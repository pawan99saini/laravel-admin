        ...
        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
            ...
            <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex flex-stack">
                ...
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <!--begin::Title-->
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Schedule
                    </h1>
                    <!--end::Title-->
                    <!--begin::Breadcrumb-->
                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table align-middle gs-0 gy-4">
                                ...
                                <tbody>
                                    @foreach($data as $val)
                                            @php 
                                            $start = \Carbon\Carbon::parse($val->shift->start_time);
                                            $end = \Carbon\Carbon::parse($val->shift->start_time);
                                            $minutes = $end->diffInMinutes($start); // 226

                                            @endphp
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            <td>{{ $val->user_schedule->first_name}}</td>
                                            <td><span style="border-style:dotted;border-color:green;pdding:10px">{{ $val->shift->start_time.'-'.$val->shift->end_time  }}</span></td>
                                            @if (Auth::user()->hasRole('admin'))

                                            <td class="text-end">
																
																<a href="{{ URL::to('schedule/' . $val->id . '/edit') }}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                                                ...
                                                                <form action="{{ route('schedule.destroy', $val->id) }}" method="POST" id="delete-{{$val->id}}">
  @csrf
  @method("DELETE")
